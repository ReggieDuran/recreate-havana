 ...
 * @return array
 */
function havana_body_classes( $classes ) {
...
add_filter( 'body_class', 'havana_body_classes' );
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function havana_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( __( 'Page %s', 'havana' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'havana_wp_title', 10, 2 );
	 ...
	 */
	function havana_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'havana_render_title' );
endif;

function havana_create_social_icons() {
...
if ( ! function_exists( 'havana_hex2rgb' ) ) {
	function havana_hex2rgb($hex) {
	   $hex = str_replace("#", "", $hex);
...
function havana_titan_custom_css() {
    if ( ! class_exists( 'TitanFramework' ) ) {
	    return;
	}
...
add_action( 'wp_head', 'havana_titan_custom_css' );
 ...
 */
function havana_the_site_logo( $html, $logo, $size ) {
	
	if ( empty( $logo ) ) {
		return '<a href="' . esc_url( home_url( '/' ) ) . '" class="site-logo-link" rel="home"><img class="site-logo attachment" width="160" src="' . get_template_directory_uri() . '/images/logo.png" title="havana WordPress Theme"/></a>';
	}
	if ( empty( $logo['url'] ) ) {
		return '<a href="' . esc_url( home_url( '/' ) ) . '" class="site-logo-link" rel="home"><img class="site-logo attachment" width="160" src="' . get_template_directory_uri() . '/images/logo.png" title="havana WordPress Theme"/></a>';
	}
...
add_filter( 'jetpack_the_site_logo', 'havana_the_site_logo', 10, 3 );
 ...
 */
add_action( 'cs_predetermineReplacements', 'me_theme_custom_sidebars_determine_replacements' );
function me_theme_custom_sidebars_determine_replacements( $defaults ) {
